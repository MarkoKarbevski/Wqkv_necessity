...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.reduce_query_weights = not self.config.use_query_weights

        # key, query, value projections for all heads, but in a batch
        if self.reduce_query_weights:
    ...
    def forward(self, x):
        ...
        if self.reduce_query_weights:
            ...
            q = x.view(B, T, self.num_heads, C // self.num_heads).transpose(1, 2) # (B, nh, T, hs)

        else:
            # reduce the query weights to save memory, as per https://arxiv.org/abs/2305.13245
