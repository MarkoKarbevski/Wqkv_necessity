...
#from config_original_untiedw import model_args
from config_reduced_untiedw import model_args
...
wandb_project = 'owt'
wandb_run_name = 'gpt2_reduced' + '_run_' + str(time.time())
# data
dataset = 'openwebtext'
batch_size = 12
...
train_collector = torch.zeros((max_iters, 3), device="cuda:0",dtype=torch.float32)
eval_train_collector = torch.zeros((max_iters//eval_interval, 3), device="cuda:0", dtype=torch.float32)
eval_val_collector = torch.zeros((max_iters//eval_interval, 3), device="cuda:0", dtype=torch.float32)
...
def estimate_loss(iter_num=0):
    ...
    torch.save(eval_train_collector, os.path.join(wandb_dir, "eval_train_tensor.pt"))
    torch.save(eval_val_collector, os.path.join(wandb_dir, "eval_val_tensor.pt"))
    torch.save(train_collector, os.path.join(wandb_dir, "tensor.pt"))
...
# training loop
X, Y = get_batch('train', it = 0, microstep=0) # fetch the very first batch
t0 = time.time()
